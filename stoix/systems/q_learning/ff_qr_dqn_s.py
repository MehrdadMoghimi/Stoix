...
from stoix.utils.loss import utility_quantile_q_learning
...
from stoix.networks.base import StockFeedForwardActor as Actor
from stoix.utils import make_env as environments
from stoix.utils.logger import LogEvent, StoixLogger
from stoix.utils.total_timestep_checker import check_total_timesteps


# For more generality, this should be configurable via Hydra,
# e.g., by specifying a string name that maps to a function.
# For now, we'll hardcode it.
def utility_f(x: jnp.ndarray) -> jnp.ndarray:
    """Example utility function f(x) = -|x|."""
    return -jnp.abs(x)
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _q_loss_fn(
                ...
                stock_t = transitions.next_obs.stock
                q_loss = utility_quantile_q_learning(
                    q_dist_tm1,
                    quantiles,
                    a_tm1,
                    r_t,
...
class EvalActorWrapper:
    ...
    def apply(self, params: FrozenDict, x: StockObservation) -> distrax.EpsilonGreedy:
...
@hydra.main(
    ...
    config_name="default_ff_qr_dqn_s.yaml",
    version_base="1.2",
)
